#!/usr/bin/env bash
ENDOFNAMEFILE=$1
ENDOFNAMEFILE="$ENDOFNAMEFILE"
NSMAGTSCALE=$2 #Magnetic field decay timescale in Myr
NSMAGTSCALE="$NSMAGTSCALE"
echo "tau_d=$NSMAGTSCALE"
...
#SEVNPATH
#-------------------------------
SEVN="/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie" #Complete path to the SEVN folder
...
#-------------------------------
#if [ "$cmdargument" = "" ]; then
LISTBINBEGIN="/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/listBin"
LISTBINEND=".dat"
LISTBIN="$LISTBINBEGIN$ENDOFNAMEFILE$LISTBINEND" #Complete path to input file (list of binaries or single stars)
#else
#  LISTBIN=$cmdargument
#fi
...
#OUTPUT
#-------------------------------
OUTPATHBEGIN="/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/sevn_output"
OUTPATHEND="/"
OUTPATH="$OUTPATHBEGIN$ENDOFNAMEFILE$OUTPATHEND" #Complete path to the output folder (the folder will be automatically created or cleaned if it already exists)
...
BCOL="Semimajor:Eccentricity:BEvent:BWorldtime" #Additional columns to print in the output file for binary stellar evolution runs. Default is empty, but any property of binary stars can be added (check names in the BinaryProperty class)
...
#-------------------------------
#PRESCRIPTIONS
#-----------std::cout << "Magnetic field is  : " << B << " T" << std::endl;--------------------
...
CIRCMODE="angmom" #Prescriptions for orbit circularisation at onset of RLO - [periastron*] [periastron_full] [angmomg] [semimajor] [disabled]
...
GWONLYBCO="true" #If true activate the GW orbital decay on for binary compact objects
#-------RLO-------#
...
RLOSTABILITY="qcrit_hurley"  #"Option for RLO mass transfer stability
...
#NSMAGTSCALE="0.1" #Magnetic field decay timescale in Myr
NSMAGMSCALE="0.025" #Magnetic field decay mass-scale in Msun
...
NSDETACC="true" #Activate NS detailed accretion handling accounting for change in magnetic field and spin due to the mass accretion
RLMODE="hurley_rl_propeller"
#-------BH-------#
...
WRTOPHE="false" #if true transform the star to pureHe as soon as it is flagged as WR (see parameter star_tshold_WR_envelope)
# -------------------------------
#Other parameters
#-------------------------------
MAXREP="100" #Maximum number of repetitions allowed in the sse and bse. If we reach this number an error is raised
...
FORCERLIN="false" #If true interpolate the Radius using a linear scaling (default uses log scale)
# -------------------------------
#Timestep
#-------------------------------
...
TSSPINBIN="true" #If true take into account the variation (BSE only) of OmegaSpin in the adaptive timestep
TSNSSPIN="true" #If true take into account the variation of OmegaRem for NS in the adaptive timestep. It should be set to true if interested on pulsars
CKSTALLING="true" #If true check stalling stars. If the elapsed evolution time is larger than 20s an error is thrown
CKSTALLINGTIME="30" #Time in seconds (units granularity) after which the system is considered stalled and the evolution is stopped and an error is raised)
...
-turn_WR_to_pureHe $WRTOPHE \
-ns_detailed_accretion $NSDETACC \
-ns_magnetic_tscale $NSMAGTSCALE \
-end_of_name_file $ENDOFNAMEFILE \
...
-check_stalling_time $CKSTALLINGTIME \
-ev_R_linear_interp $FORCERLIN"
