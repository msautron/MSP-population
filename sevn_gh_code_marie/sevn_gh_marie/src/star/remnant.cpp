...
double NSrem::OmegaRem(_UNUSED Star *s) {
	...
	//FILE *data_period_pulsars;
	//FILE *data_p_dot_pulsars;
	//FILE *data_alpha_pulsars;
	//FILE *data_beta_pulsars;
	//char cmd4[500];
        //char cmd5[500];
	//char cmd6[500];
	//char cmd7[500];
	...
	//end_name_file=s->get_svpar_num("end_of_name_file");
        //sprintf(eonf,"%ld",end_name_file);
	//sprintf(cmd4, "/home/mepietrin/sevn_gh/SEVN_run/data_period_pulsars%s.txt",eonf);
	//sprintf(cmd5, "/home/mepietrin/sevn_gh/SEVN_run/data_alpha_pulsars.txt",eonf);
	//sprintf(cmd6, "/home/mepietrin/sevn_gh/SEVN_run/data_beta_pulsars.txt",eonf);
	//sprintf(cmd7, "/home/mepietrin/sevn_gh/SEVN_run/data_p_dot_pulsars%s.txt",eonf);
        //data_beta_pulsars=fopen(cmd6,"a+");
	//data_period_pulsars=fopen(cmd4,"a+");
	//data_alpha_pulsars=fopen(cmd5,"a+");
	//data_p_dot_pulsars=fopen(cmd7,"a+");
	cpt+=1.0;
	//if(cpt%40==0){
	//fprintf(data_period_pulsars,"%e %e\n",fabs(P),s_tps);
       
	//}	
	//if(cpt%40==0){
        //fprintf(data_p_dot_pulsars,"%e %e\n",fabs(Pdot),s_tps);

	//}
	/*if(cpt%30==0){
                fprintf(data_alpha_pulsars,"%e %e\n",acos(cosalpha),s_tps);

        }
	if(cpt%30==0){
                fprintf(data_beta_pulsars,"%e %e\n",beta,s_tps);

        }
*/
	//fprintf(data_p_dot_pulsars,"\n");
	//fprintf(data_p_dot_pulsars,"%e %e\n",fabs(Pdot),s_tps);
	//fprintf(data_period_pulsars,"\n");
	//fprintf(data_period_pulsars,"%e %e\n",fabs(P),s_tps);
	//fclose(data_period_pulsars);
	//fclose(data_alpha_pulsars);
	//fclose(data_beta_pulsars);
	//fclose(data_p_dot_pulsars);
	std::cout << "P is : " << P << " in s, and alpha is : " << alpha*180/M_PI << " in degrees, and Pdot is : " << Pdot << " in s/s, Age = " << age(s) << " in Myr " << std::endl;
	acc_activate(false);
/*     
 if(age(s)=real_age and ...){
 *      FILE *data_ms_pulsars;
        ...
        end_name_file=s->get_svpar_num("end_of_name_file");
        sprintf(eonf,"%ld",end_name_file);
        sprintf(cmd,"/home/mepietrin/sevn_gh/SEVN_run/data_pop_ms%s.txt",eonf);
        sprintf(cmd2,"python3 /home/mepietrin/sevn_gh/SEVN_run/get_info_binary%ld.py",end_name_file);
        sprintf(cmd3,"/home/mepietrin/sevn_gh/SEVN_run/data_taud%s.txt",eonf);
...
*/
	...
	end_name_file=s->get_svpar_num("end_of_name_file");
	sprintf(eonf,"%ld",end_name_file);
	sprintf(cmd,"/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/data_pop_ms%s.txt",eonf); 
	//sprintf(cmd4,"/home/mepietrin/sevn_gh/SEVN_run/data_mass_type%s.txt",eonf);
	sprintf(cmd2,"python3 /home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/get_info_binary.py %ld",end_name_file);
	sprintf(cmd3,"/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/data_taud%s.txt",eonf);
       	sprintf(cmd1,"/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/data_mass_et_type%s.txt",eonf);
        //sprintf(cmd5,"/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/data_end_mass_type%s.txt",eonf);
	//data_mass_type = fopen(cmd1,"r");
	//info_mass_et_type_wd=fopen(cmd5,"a+");
	data_ms_pulsars=fopen(cmd,"w+");
	/*char data[500];
	while (fgets(data, sizeof(data), data_mass_type) !=NULL){
		
		fputs(data, info_mass_et_type_wd);
	}*/


	data_taud=fopen(cmd3,"w+");
	fprintf(data_ms_pulsars,"%e %e %e %e %e %e %e %e %e %e %e %e %e %e",fabs(P),fabs(Pdot),Edot,acos(cosalpha),Bmag(s)*1e-4,age(s)*1e6*365*24*3600,Pinit,B0*1e-4,alpha0,tps_acc,spin_up_val,s->getp(Mass::ID),M0,ell);
	fprintf(data_taud,"%e\n",tau_magnetic);
	fclose(data_ms_pulsars);
	fclose(data_taud);
	//fclose(info_mass_et_type_wd);
	//fclose(data_mass_type);
	system(cmd2);
...
void NSrem::default_initialiser(Star *s) {
    ...
    end_name_file=s->get_svpar_num("end_of_name_file");
    sprintf(eonf,"%ld",end_name_file);
    sprintf(cmd,"/home/matteo.sautron/sevn_gh_code_marie/sevn_gh_marie/SEVN_run/listBin%s.dat",eonf);
    //sprintf(cmd1,"/home/mepietrin/sevn_gh/SEVN_run/info_mass_wd%s.txt",eonf);
    info_birth=fopen(cmd,"r");
    //info_mass_wd=fopen(cmd1,"w+");
    double trash;char trash_str[100];
    fscanf(info_birth,"%le%s %le %le %s %le %le %le %le %s %s %le %le %le %le",&trash,trash_str,&trash,&trash,trash_str,&trash,&trash,&trash,&trash,trash_str,trash_str,&trash,&trash,&real_age,&trash);
    //fprintf(info_mass_wd,"%e",Mass);
