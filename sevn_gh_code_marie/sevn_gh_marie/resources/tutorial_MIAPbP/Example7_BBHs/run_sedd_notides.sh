...
#OUTPUT
#-------------------------------
OUTPATH="sevn_output_sedd_notides" #Complete path to the output folder (the folder will be automatically created or cleaned if it already exists)
...
SCOL="Mass:MHE:MCO:Radius:Luminosity:Temperature:Phase:RemnantType:Zmet" #Additional columns to print in the output file for single stellar evolution runs. Default is empty, but any property of single stars can be added (check names in the Property class)
BCOL="BWorldtime:Semimajor:Eccentricity:BEvent:Period:GWtime" #Additional columns to print in the output file for binary stellar evolution runs. Default is empty, but any property of binary stars can be added (check names in the BinaryProperty class)
...
TIDES="disabled" #Prescriptions for tides - [simple*] [disabled]
GWMODE="peters" #Prescriptions for gravitational-wave decay - [peters*] [disabled]
RLMODE="hurley_bse" #Prescriptions for Roche-Lobe overflow and mass transfer/accretion - [hurley_rl*] [hurley_bse] [disabled]
CIRCMODE="disabled" #Prescriptions for orbit circularisation at onset of RLO - [periastron*] [periastron_full] [angmomg] [semimajor] [disabled]
...
SNPISN="farmer19" #Prescription for pair instability SN - [mapelli20*] [iorio22] [iorio22_limited] [farmer19] [disabled]
...
RLOMACCR="1" #Fraction of the mass lost by the primary that is accreted onto the secondary during RLO
RLOGAM="-1" #Angular momentum lost during RLO. [-1]: from the primary, [-2]: from the secondary, [>0]: fraction lost from the system
...
#-------ACCRETION-------#
EDDF="1000000" #Eddington factor to limit accretion on a compact object (>1 means super-Eddington)
#-------SN-------#
...
#-------CE-------#
CEALPHA="1" #alpha in binding energy (Eq. 73 in Hurley02)
