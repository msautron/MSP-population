...
TABLES="${SEVN}/tables/SEVNtracks_mist_AGB" #Complete path to look-up tables
...
#OUTPUT
#-------------------------------
OUTPATH="sevn_output_period_mist/" #Complete path to the output folder (the folder will be automatically created or cleaned if it already exists)
...
BCOL="BWorldtime:Semimajor:Eccentricity:Period:BEvent" #Additional columns to print in the output file for binary stellar evolution runs. Default is empty, but any property of binary stars can be added (check names in the BinaryProperty class)
...
TABCONV="false" #If true estimate the properties of the convective envelope using the tables (xxxconv.dat)
...
TABRHE="false" #If true estimate the properties of the HE core radius using the tables (rhe.dat)
TABRCO="false" #If true estimate the properties of the CO core radius using the tables (rhe.dat)
