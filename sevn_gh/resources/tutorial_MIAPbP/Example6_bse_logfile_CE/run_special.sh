...
#-------------------------------
if [ "$cmdargument" = "" ]; then
  LISTBIN="IC_special.dat" #Complete path to input file (list of binaries or single stars)
else
  LISTBIN=$cmdargument
fi
...
#OUTPUT
#-------------------------------
OUTPATH="sevn_output_special" #Complete path to the output folder (the folder will be automatically created or cleaned if it already exists)
...
SCOL="Mass:MHE:MCO:Radius:Luminosity:Temperature:Phase:PhaseBSE:RemnantType:Zmet" #Additional columns to print in the output file for single stellar evolution runs. Default is empty, but any property of single stars can be added (check names in the Property class)
